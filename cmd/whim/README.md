...
| `go tool whim java` | the editor in Java (`jeditor/`, doc/JAVA.md): the core cut from `src/whim-vim.c` (or FILE), written as `Editor.java`, compiled with jeditor's runtime, host and glue into `lib/java/classes` (`--out DIR`), and `bin/whim-java`, a launcher script (`DIR/whim-java` with `--out`); `make jeditor.jar` packs the classes as `./jeditor.jar` |
| `go tool whim clj` | the editor in Clojure (`cljeditor/`, doc/CLOJURE.md): the core cut from `src/whim-vim.c` (or FILE) and written as the namespace `whim.editor` by the Clojure backend (`--editor F`: the namespace in F instead), AOT-compiled with cljeditor's glue and launcher on jeditor's runtime and host, merged with Clojure's jars into `lib/clj/whim-clj.jar` (`--out DIR`) with its AOT cache, and `bin/whim-clj`, a launcher script; `--jar F` writes the same jar at F (`make cljeditor.jar`) |
