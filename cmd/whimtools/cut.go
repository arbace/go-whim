...
func runRetire(args []string) int {
	if len(args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: whimtools retire <file> <command>...")
		return 1
	}
...
func runDroplocal(args []string) int {
	if len(args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: whimtools droplocal <file> <field>...")
		return 1
	}
	path := args[0]
	text, err := os.ReadFile(path)
...
func runDropoptions(args []string) int {
	...
	if len(rest) < 2 {
		fmt.Fprintln(os.Stderr, "usage: whimtools dropoptions <file> <option-name>...")
		return 1
	}
...
func runDropopts(args []string) int {
	if len(args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: whimtools dropopts <file> <-x|--long>...")
		return 1
	}
	path := args[0]
	text, err := os.ReadFile(path)
