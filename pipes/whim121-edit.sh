...
# -- no surviving row carries that name -- and pipes/whim121-check.sh proves it again
...
# format string and the name move together, as pipes/whim95-edit.sh's `[RO]` did,
...
work=${1:?usage: whim121-edit.sh <work-dir> <state-dir>}
state=${2:?usage: whim121-edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# tools/phaserun.sh sweeps next, then runs pipes/whim121-check.sh.
