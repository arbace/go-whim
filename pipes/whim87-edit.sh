...
# flags, exactly as pipes/whim85-edit.sh does it: pipes/whim87-check.sh requires the
# OLD binary to enter Ex mode and the new one to refuse, which is the difference
# between a probe and a formality.
set -eu

work=${1:?usage: whim87-edit.sh <work-dir> <state-dir>}
state=${2:?usage: whim87-edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# to find -- it raises rather than reporting nothing (pipes/whim85-edit.sh says the
...
# tools/phaserun.sh sweeps next, then runs pipes/whim87-check.sh.
