...
#
# below the includes, beside the twelve.  pipes/whim115-check.sh measures that it holds
...
# core -> host signature takes scalars and byte buffers only (pipes/whim111-check.sh),
...
# or when.  pipes/whim115-check.sh instruments every clock read on BOTH binaries and
...
work=${1:?usage: whim115-edit.sh <work-dir> <state-dir>}
state=${2:?usage: whim115-edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# tools/phaserun.sh sweeps next, then runs pipes/whim115-check.sh.
