...
# mask and the two-row table, and pipes/whim100-check.sh measures exactly that: the same
...
work=${1:?usage: whim100-edit.sh <work-dir> <state-dir>}
state=${2:?usage: whim100-edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# tools/phaserun.sh sweeps next, then runs pipes/whim100-check.sh.
