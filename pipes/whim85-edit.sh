...
# not evidence, so pipes/whim85-check.sh measures the removed behaviour directly, in
...
work=${1:?usage: whim85-edit.sh <work-dir> <state-dir>}
state=${2:?usage: whim85-edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# tools/phaserun.sh sweeps next, then runs pipes/whim85-check.sh.
