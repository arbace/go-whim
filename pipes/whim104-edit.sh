...
# pipes/whim104-check.sh states the claim as one `comm` with an empty `arrived` side
...
# in this editor is built in -- and pipes/whim104-check.sh pins both halves of it, the
...
work=${1:?usage: whim104-edit.sh <work-dir> <state-dir>}
state=${2:?usage: whim104-edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# tools/phaserun.sh sweeps next, then runs pipes/whim104-check.sh.
