...
func (g *gen) writeBodies(dir string, skip map[string]bool) error {
	...
	out.WriteString("// Code generated by tx/skel from editor.c: the functions' bodies.\n\npackage main\n\n")
	n, total := 0, 0
	whys := map[string]int{}
	...
	fmt.Fprintf(os.Stderr, "skel: %d of %d functions written\n", n, total)
	for i, k := range ks {
		if i == 25 {
			break
		}
	...
	if err := os.WriteFile(filepath.Join(dir, "bodies.go"), []byte(out.String()), 0o644); err != nil {
		return err
	}
	return os.WriteFile(filepath.Join(dir, "bodies.txt"), []byte(report.String()), 0o644)
}
