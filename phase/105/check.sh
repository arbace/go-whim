...
# See phase/105/edit.sh, and GOALS.md II.4c.
#
# Usage: phase/105/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/105/edit.sh and the sweep tools/phaserun.sh runs between them, and
...
# THE BODY IS GO: internal/check/whim105.go and internal/check/whim105probes.go, run through tools/st.sh.
...
work=${1:?usage: phase/105/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/105/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim105 "$work" "$state"
