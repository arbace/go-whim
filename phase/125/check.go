...
// Whim phase 125's proof -- the swap file's residue.  See phase/125/edit.go.
//
// THE DECLARED DELTA IS NOTHING AT ALL, AND IT IS TWO DIFFERENT KINDS AT ONCE.  Four
// Part II phases have declared nothing before, for four different reasons (CLAUDE.md,
// *Verification tiers*), and this one phase is two of them in the same edit:
//
// THE NEGATIVE-BLOCK HALF IS PHASE 92'S KIND -- code that COULD NOT RUN.  The chain the
...
//
// THE BLOCK-ZERO HALF IS PHASE 95'S KIND -- code that RUNS and the instrument cannot
...
// move it.
//
// THE FANOUT CHANGES, AND THAT IS WHAT PHASE 123 IS FOR.  `pe_old_lnum` is a member of
...
//
// THIS IS THE FIRST MEMLINE CHANGE SINCE PHASE 123, which added a corpus of buffers
...
func Check(w io.Writer, args []string) error {
	...
	mid := check.ReadFile(filepath.Join(state, "edit.c"))
	declaredIn, _ := strconv.Atoi(strings.TrimSpace(check.ReadFile(filepath.Join(state, "input-lines"))))
	...
	if strings.Join(iface["old"], "\n") != strings.Join(iface["new"], "\n") {
		sym := map[string]bool{}
		for _, n := range check.Comm23(iface["old"], iface["new"]) {
			sym[n] = true
		}
	...
	newSize, oldSize := check.SizeOf(T("new")), check.SizeOf(filepath.Join(state, "old"))
	...
	if strings.Join(uOld, "\n") != strings.Join(uNew, "\n") {
		sym := map[string]bool{}
		for _, n := range check.Comm23(uOld, uNew) {
			sym[n] = true
		}
	...
	marked := 0
	for _, n := range files {
		if scan(check.ReadFile(filepath.Join(T("SC-probe"), n))) > 0 {
			marked++
		}
	}
	...
	for _, extra := range []string{"ex-probe.txt", "argv-probe.txt", "stress-probe.txt"} {
		scan(check.ReadFile(T(extra)))
	}
	mem := listDir(T("ML-in"))
	memMarked := 0
	for _, n := range mem {
		if scan(check.ReadFile(filepath.Join(T("ML-in"), n))) > 0 {
			memMarked++
		}
	}
	...
	for _, v := range []string{"c_open", "c_bnum", "c_root"} {
		...
		d[v] = len(check.DiffRQ(filepath.Join(T("REC-new"), "screen"), T("SC-"+v)))
	}
	...
	filesMatching := func(dir string, re *regexp.Regexp) int {
		...
		for _, name := range listDir(dir) {
			if re.MatchString(check.ReadFile(filepath.Join(dir, name))) {
				n++
			}
		}
		return n
