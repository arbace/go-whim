#!/bin/sh
# Whim phase 125's proof -- the swap file's residue.  See phase/125/edit.sh.
#
# Usage: phase/125/check.sh <work-dir> <state-dir>    (run from the repository root)
#
...
# THE BODY IS GO: internal/check/whim125.go, run through tools/st.sh.
...
work=${1:?usage: phase/125/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/125/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim125 "$work" "$state"
