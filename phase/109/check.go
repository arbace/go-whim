...
func Check(w io.Writer, args []string) error {
	...
	fatal := func(head string, logPath string, n int) error {
		r.Say("%s", head)
		lines := strings.Split(check.ReadFile(logPath), "\n")
		for i, l := range lines {
			if i >= n {
				break
			}
...
`
	...
	var mm []string
	for _, l := range strings.Split(check.ReadFile(filepath.Join(state, "minmax.txt")), "\n") {
	...
	wx0 := check.ReadFile(filepath.Join(tmp, "w.x0"))
	if wx0 != "" {
		return fatal("the sixteen static_asserts compiled but were not silent:",
			filepath.Join(tmp, "w.x0"), 12)
	}
	if !strings.Contains(check.ReadFile(filepath.Join(tmp, "w.x0bad")), "static assertion failed") {
	...
	for _, m := range mis {
		if !strings.Contains(check.ReadFile(filepath.Join(tmp, "w."+m.Name)), m.want) {
	...
	if check.ReadFile(filepath.Join(tmp, "w.tag23")) != "" {
	...
	for _, s := range []string{"c11", "c17"} {
		if !strings.Contains(check.ReadFile(filepath.Join(tmp, "w.tag"+s)),
	...
	oldBin := check.ReadFile(filepath.Join(state, "old"))
	if check.ReadFile(filepath.Join(tmp, "r0")) != oldBin {
	...
	newBin := check.ReadFile(filepath.Join(tmp, "new"))
	if newBin == oldBin {
	...
	if check.ReadFile(filepath.Join(tmp, "swap")) == newBin {
	...
	r.Say("THE CLOCK IS THE ONLY THING THAT CHANGES CODE, as an equality: with it ALONE "+
		...
		check.SizeOf(filepath.Join(state, "old")), check.SizeOf(filepath.Join(tmp, "new")))
	...
	go func() {
		defer wgR.Done()
		recErr[2] = check.RecCmd("sh", "tools/st.sh", "zcases", filepath.Join(tmp, "swap"),
