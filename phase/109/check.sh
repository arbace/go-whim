...
# See phase/109/edit.sh, and GOALS.md II.4c.
#
# Usage: phase/109/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/109/edit.sh and the sweep tools/phaserun.sh runs between them, and
...
# THE BODY IS GO: internal/check/whim109.go, run through tools/st.sh.
...
work=${1:?usage: phase/109/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/109/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim109 "$work" "$state"
