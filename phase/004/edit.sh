#!/bin/sh
# Whim phase 4 -- the binary's name stops choosing what it does.  See GOAL.md.
#
# Usage: phase/004/edit.sh <work-dir> <state-dir>       (run from the repository root)
#
...
work=${1:?usage: phase/004/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

# --- cut the entry point --------------------------------------------------
tools/st.sh noargv0 "$f"

# tools/phaserun.sh sweeps next, then runs phase/004/check.sh.
