#!/bin/sh
# Whim phase 84 -- the stack protector goes.  See GOAL.md.
#
# Usage: phase/084/make.sh <work-dir>       (run from the repository root)
#
...
work=${1:?usage: phase/084/make.sh <work-dir>}
f="$work/whim-vim.c"
mk="$work/Makefile"
...
if [ "$(sha256sum "$f" | cut -c1-64)" != "$sha_in" ]; then
    echo "  source       whim-vim.c CHANGED -- this phase edits the makefile only"
    exit 1
fi
