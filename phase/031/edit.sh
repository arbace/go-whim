#!/bin/sh
# Whim phase 31 -- file-name modifiers.  See GOAL.md.
#
# Usage: phase/031/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
work=${1:?usage: phase/031/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

# --- cut the entry points -------------------------------------------------
tools/st.sh nofnamemod "$f"

# tools/phaserun.sh sweeps next, then runs phase/031/check.sh.
