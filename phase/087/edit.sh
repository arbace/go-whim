...
#
# Usage: phase/087/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
# flags, exactly as phase/085/edit.sh does it: phase/087/check.sh requires the
# OLD binary to enter Ex mode and the new one to refuse, which is the difference
# between a probe and a formality.
set -eu

work=${1:?usage: phase/087/edit.sh <work-dir> <state-dir>}
state=${2:?usage: phase/087/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# to find -- it raises rather than reporting nothing (phase/085/edit.sh says the
...
# tools/phaserun.sh sweeps next, then runs phase/087/check.sh.
