#!/bin/sh
# Whim phase 45 -- no :drop.  See GOAL.md.
#
# Usage: phase/045/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
work=${1:?usage: phase/045/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

tools/st.sh retire "$f" drop

# tools/phaserun.sh sweeps next, then runs phase/045/check.sh.
