...
// which child a line is in.  A `whim-vim` with
//
...
//
// and `rm -rf .cache/r0` alone is not enough -- `phase/083/check.go` refuses a set that
...
func Check(w io.Writer, args []string) error {
	...
	var ext []string
	for _, l := range check.Lines(string(x)) {
	...
	setup := func(dir, text string) {
		...
		check.Put(T(dir, "whim-vim.c"), text)
		check.Put(T(dir, "Makefile"), mk)
	}
	setup("probe", probed)
	...
	for _, e := range errs {
		if e != nil {
			(&check.Rep{Tag: "ablefail", W: w}).Say("an instrumented or control copy did not build -- the break is wrong, not the corpus")
			return harness.ErrReported
		}
	}
	...
	quiet := func(args ...string) error {
		e := check.RecCmd(append([]string{"tools/st.sh"}, args...)...)
		check.RecReport(w, e)
		return e
	}
	for _, a := range [][]string{{"zmemline", T("probe", "whim-vim"), T("probe-mem")},
		{"zcases", T("probe", "whim-vim"), T("probe-screen")},
		{"zmemline", T("reprobe", "whim-vim"), T("reprobe-mem")}} {
		if quiet(a...) != nil {
			return harness.ErrReported
		}
	}
	...
	run1 := T("run1")
	(&check.Rep{Tag: "instrument", W: w}).Say("%d cases, %d memline cases, %d commands, %d command lines, %d pty scenarios, %d terminals: 3 identical runs",
		check.DirCount(filepath.Join(run1, "screen")), check.DirCount(filepath.Join(run1, "memline")),
		check.CountHeaders(filepath.Join(run1, "ref-excmds.txt")), check.CountHeaders(filepath.Join(run1, "ref-argv.txt")),
		check.CountHeaders(filepath.Join(run1, "ref-pty.txt")), check.CountLines([]byte(check.ReadFile(filepath.Join(run1, "ref-term.txt")))))

	// --- 6. the instrument can fail, and the one it joins cannot ------------
	for _, c := range []string{"descent", "lineadd", "cache", "reshape", "clock"} {
		if quiet("zmemline", T(c, "whim-vim"), T("mem-"+c)) != nil || quiet("zcases", T(c, "whim-vim"), T("scr-"+c)) != nil {
			return harness.ErrReported
		}
	}
	...
	(&check.Rep{Tag: "source", W: w}).Say("whim-vim.c unchanged, %d lines: r40 is its input's tree, and only the instrument moved", check.CountLines([]byte(check.ReadFile(f))))
	return nil
}
...
`
	...
	os.MkdirAll(d, 0o755)
	check.Put(filepath.Join(d, "d.c"), prog)
...
func z40Read(d string) map[string]string {
	...
	e, _ := os.ReadDir(d)
	for _, x := range e {
		Out[x.Name()] = strings.ToValidUTF8(check.ReadFile(filepath.Join(d, x.Name())), "�")
	}
	return Out
}
	...
	for _, c := range corrupt {
		if len(byscr[c]) > 0 {
			bad = append(bad, fmt.Sprintf("the %s control moved %d of the %d SCREEN cases (%s), so the old corpus is not blind to it and this phase has the wrong premise",
				c, len(byscr[c]), len(baseScr), strings.Join(check.Head(byscr[c], 5), " ")))
		}
	}
