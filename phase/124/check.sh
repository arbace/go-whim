#!/bin/sh
# Whim phase 124, the check -- freeing is free.  See phase/124/edit.sh, and
# GOALS.md's charter bullet "A GARBAGE COLLECTOR IS ASSUMED FROM HERE ON".
#
# Usage: phase/124/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/124/edit.sh and the sweep tools/phaserun.sh runs between them, and
...
work=${1:?usage: phase/124/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/124/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim124 "$work" "$state"
