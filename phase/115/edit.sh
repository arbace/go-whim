...
# See GOALS.md II.4c, GOALS.md, phase/109/edit.sh (which wrote the core's libc
# prototypes) and phase/111/edit.sh (the scalar clock, whose musl_now_ms is this
# phase's sibling).
#
# Usage: phase/115/edit.sh <work-dir> <state-dir>     (run from the repository root)
#
...
#
# below the includes, beside the twelve.  phase/115/check.sh measures that it holds
...
# core -> host signature takes scalars and byte buffers only (phase/111/check.sh),
...
# or when.  phase/115/check.sh instruments every clock read on BOTH binaries and
...
work=${1:?usage: phase/115/edit.sh <work-dir> <state-dir>}
state=${2:?usage: phase/115/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# tools/phaserun.sh sweeps next, then runs phase/115/check.sh.
