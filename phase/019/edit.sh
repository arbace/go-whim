#!/bin/sh
# Whim phase 19 -- the terminal is what the build says.  See GOAL.md.
#
# Usage: phase/019/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
work=${1:?usage: phase/019/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

# --- cut the entry points -------------------------------------------------
tools/st.sh noterm "$f"

# tools/phaserun.sh sweeps next, then runs phase/019/check.sh.
