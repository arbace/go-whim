...
# See phase/110/edit.sh, GOALS.md II.4c and .claude/briefs/zero-reorg.md 5.
#
# Usage: phase/110/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/110/edit.sh and the sweep tools/phaserun.sh runs between them, and
...
# THE BODY IS GO: internal/check/whim110.go, run through tools/st.sh.
...
work=${1:?usage: phase/110/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/110/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim110 "$work" "$state"
