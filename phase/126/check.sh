#!/bin/sh
# Whim phase 126 -- the proof.  See phase/126/edit.sh for what the phase does.
#
# Usage: phase/126/check.sh <work-dir> <state-dir>    (run from the repository root)
#
...
# THE BODY IS GO: internal/check/whim126.go, run through tools/st.sh.
...
work=${1:?usage: phase/126/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/126/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim126 "$work" "$state"
