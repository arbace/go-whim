...
// Whim phase 126 -- the proof.  See phase/126/edit.go for what the phase does.
//
...
func Check(w io.Writer, args []string) error {
	...
	corpus := func(binary, tag string) (map[string]ev5, error) {
		Out := T("mem-" + tag)
		os.RemoveAll(Out)
		if e := check.RecCmd("sh", "tools/st.sh", "zmemline", binary, Out); e != nil {
			check.RecReport(w, e)
	...
	nrec := len(check.WalkFiles(T("rec-old")))
	...
	carry := func(root string) (int, int) {
		...
		for _, Rel := range check.WalkFiles(root) {
			tot++
			if strings.Contains(check.ReadFile(filepath.Join(root, Rel)), "MARK-E323") {
				hit++
			}
		}
		return hit, tot
