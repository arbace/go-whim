...
# See phase/120/edit.sh, and GOALS.md.
#
# Usage: phase/120/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/120/edit.sh and the sweep tools/phaserun.sh runs between them, and
...
# THE BODY IS GO: internal/check/whim120.go, run through tools/st.sh.
...
work=${1:?usage: phase/120/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/120/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim120 "$work" "$state"
