...
#
# Usage: phase/127/check.sh <work-dir> <state-dir>   (run from the repository root)
#
...
# THE BODY IS GO: internal/check/whim127.go, run through tools/st.sh.
...
work=${1:?usage: phase/127/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/127/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim127 "$work" "$state"
