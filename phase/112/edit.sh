...
#
# Usage: phase/112/edit.sh <work-dir> <state-dir>     (run from the repository root)
#
...
# this edit does, and what phase/112/check.sh then does again from this machine's
...
work=${1:?usage: phase/112/edit.sh <work-dir> <state-dir>}
state=${2:?usage: phase/112/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

# The flags are read out of the boundary's makefile rather than written here a second
# time: zero's compile line is the boundary's (GOALS.md core rule 8).  The check needs
...
# tools/phaserun.sh sweeps next, then runs phase/112/check.sh.
