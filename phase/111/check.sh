...
# See phase/111/edit.sh, and GOALS.md II.4c.
#
# Usage: phase/111/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/111/edit.sh and the sweep tools/phaserun.sh runs between them, and
...
# monotonic one is 32-bit arithmetic, and phase/111/edit.sh states it.

# THE BODY IS GO: internal/check/whim111.go, run through tools/st.sh.
...
work=${1:?usage: phase/111/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/111/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim111 "$work" "$state"
