...
# See phase/097/edit.sh, and GOALS.md.
#
# Usage: phase/097/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/097/edit.sh and the sweep tools/phaserun.sh runs between them,
...
work=${1:?usage: phase/097/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/097/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim97 "$work" "$state"
