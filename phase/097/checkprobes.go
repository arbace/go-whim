...
func w97Probes(r *check.Rep, old, bin string) error {
	...
	if !o.ok || !n.ok {
		...
		if !strings.HasPrefix(n.first, strings.Repeat("X", 19)+"aaa") {
			...
			fail = append(fail, fmt.Sprintf("t_CF overflow: the new binary drew %s and not nineteen X and the buffer text -- sizeof(buf) is 20, so nineteen characters and a NUL is exactly what fits", check.CutilRepr(shown)))
		}
		if o.Text == n.Text {
