...
# See GOAL.md.
#
# Usage: phase/104/edit.sh <work-dir> <state-dir>     (run from the repository root)
#
# GOALS.md II.4c's second step, and the half of it that is not the screen:
...
# phase/104/check.sh states the claim as one `comm` with an empty `arrived` side
...
# in this editor is built in -- and phase/104/check.sh pins both halves of it, the
...
# be false: `mch_write()` still holds one `write(1, ...)`, which is GOALS.md II.4c's
# remaining step and not this one.  What IS assertable, and what phase/104/check.sh
...
# at q103.
#
# THE COUNTING TRAP, WHICH IS PHASE 102'S `exit` TRAP WITH DIFFERENT WORDS.
...
work=${1:?usage: phase/104/edit.sh <work-dir> <state-dir>}
state=${2:?usage: phase/104/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# tools/phaserun.sh sweeps next, then runs phase/104/check.sh.
