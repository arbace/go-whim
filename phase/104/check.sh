...
# See phase/104/edit.sh, and GOALS.md.
#
# Usage: phase/104/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/104/edit.sh and the sweep tools/phaserun.sh runs between them,
...
#               deliberately adds nothing to its VOCAB (phase/104/edit.sh says why:
...
# THE BODY IS GO: internal/check/whim104.go, run through tools/st.sh.
...
work=${1:?usage: phase/104/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/104/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim104 "$work" "$state"
