...
# See phase/098/edit.sh, and GOALS.md.
#
# Usage: phase/098/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/098/edit.sh and the sweep tools/phaserun.sh runs between them,
...
# THE BODY IS GO: internal/check/whim98.go and internal/check/whim98probes.go, run through tools/st.sh.
...
work=${1:?usage: phase/098/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/098/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim98 "$work" "$state"
