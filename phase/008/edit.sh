#!/bin/sh
# Whim phase 8 -- :! keeps its name and loses its process.  See GOAL.md.
#
# Usage: phase/008/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
work=${1:?usage: phase/008/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

# --- cut above the temp file ----------------------------------------------
tools/st.sh noshellout "$f"

# tools/phaserun.sh sweeps next, then runs phase/008/check.sh.
