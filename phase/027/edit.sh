#!/bin/sh
# Whim phase 27 -- `[[=a=]]` stops meaning "a with any accent".  See GOAL.md.
#
# Usage: phase/027/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
work=${1:?usage: phase/027/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

# --- cut the entry points -------------------------------------------------
tools/st.sh noequiclass "$f"

# tools/phaserun.sh sweeps next, then runs phase/027/check.sh.
