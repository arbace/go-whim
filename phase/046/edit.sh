#!/bin/sh
# Whim phase 46 -- no :wall, :qall, :quitall, :wqall or :xall.  See GOAL.md.
#
# Usage: phase/046/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
work=${1:?usage: phase/046/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

tools/st.sh retire "$f" wall qall quitall wqall xall

# tools/phaserun.sh sweeps next, then runs phase/046/check.sh.
