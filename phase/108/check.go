...
// to the evidence every phase before 106 used -- A RECORDING -- with a control for
...
func Check(w io.Writer, args []string) error {
	...
	oldS := check.ReadFile(filepath.Join(state, "old.c"))
	...
	wgC.Wait()
	errC1 := check.ReadFile(filepath.Join(tmp, "e.c1"))
	...
	errC2 := check.ReadFile(filepath.Join(tmp, "e.c2"))
	errC3 := check.ReadFile(filepath.Join(tmp, "e.c3"))
	...
	if errC3 != "" || check.SizeOf(filepath.Join(tmp, "c3.o")) < 0 {
	...
	if _, err := os.Stat(bin); err == nil {
		(&check.Rep{Tag: "build", W: w}).Say("the clean did not remove whim-vim, so a " +
	...
	(&check.Rep{Tag: "build", W: w}).Say("ok, %d -> %d lines, %d bytes",
	...
	oldSize := check.SizeOf(filepath.Join(state, "old"))
	newSize := check.SizeOf(filepath.Join(tmp, "new"))
	...
	if newSize != check.SizeOf(bin) {
		return stop("the reproducible build is %d bytes and make produced %d: the two "+
			...
			"boundary", newSize, check.SizeOf(bin))
	}
	if oldSize != newSize {
	...
	a := check.ReadFile(filepath.Join(state, "old"))
	b := check.ReadFile(filepath.Join(tmp, "new"))
	if a == b {
	...
	for k, x := range []struct{ Name, bin, src string }{
		...
		go func(k int, name, b, s string) {
			defer wgR.Done()
			recErr[k] = check.RecCmd("sh", "tools/zrecord.sh", b, s, filepath.Join(tmp, "REC-"+name))
	...
	moved := func(which string) ([]string, error) {
		...
		var Out []string
		for _, n := range base {
			if check.ReadFile(filepath.Join(tmp, "REC-new", n)) != check.ReadFile(filepath.Join(d, n)) {
				Out = append(Out, n)
			}
		}
		return Out, nil
