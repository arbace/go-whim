...
# See phase/090/edit.sh, and GOALS.md.
#
# Usage: phase/090/check.sh <work-dir> <state-dir>     (run from the repository root)
#
# Runs after phase/090/edit.sh and the sweep tools/phaserun.sh runs between them,
...
# THE BODY IS GO: internal/check/whim90.go and internal/check/whim90probes.go, run through tools/st.sh.
...
work=${1:?usage: phase/090/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/090/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim90 "$work" "$state"
