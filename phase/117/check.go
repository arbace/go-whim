...
// See phase/117/edit.go, and GOALS.md II.4c.
//
// Runs after phase/117/edit.go and the sweep tools/phaserun.sh runs between them, and
...
func z34Block(t, head, need string) (string, error) {
	...
	if !strings.Contains(b, need) {
		return "", fmt.Errorf("the extracted block does not hold %s", check.CutilRepr(need))
	}
	return b, nil
}
...
func Check(w io.Writer, args []string) error {
	...
	r.Say("THE UNIT HARNESS: ga_grow_inner(), musl_memcpy(), musl_memset(), garray_T and get_keystroke's "+
		...
		"back intact", state, check.CountLines([]byte(check.ReadFile(T("run.new")))))
	...
	for _, x := range []struct{ Name, want string }{
		...
		if !strings.Contains(check.ReadFile(T("run."+x.Name)), x.want) {
	...
	if !check.Contains(uNew, "malloc") || !check.Contains(uNew, "free") {
	...
	os.WriteFile(T("c_ncopy_full.c"), []byte(check.ReadFile(T("c_nocopy.c"))), 0o644)
	...
	go func() {
		defer wgR.Done()
		ncErr = check.RecCmd("sh", "tools/st.sh", "zcases", T("c_ncopy_full"), T("SCR.nocopy"))
	}()
	wgR.Wait()
	// A recording that fails says why, and ends the check: see recjob.go.
	if check.RecReport(w, append(recErr, ncErr)...) {
		return harness.ErrReported
	}
	...
	marks := func(rec, token string) mk3 {
		...
		for _, Rel := range check.WalkFiles(rec) {
			m.files++
			if c := strings.Count(check.ReadFile(filepath.Join(rec, Rel)), token); c > 0 {
				m.hit++
				m.total += c
			}
		}
	...
	moved := len(check.DiffRQ(T("REC.new")+"/screen", T("SCR.nocopy")))
	if moved < 100 {
