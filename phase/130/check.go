...
// See phase/130/edit.go, and GOALS.md.
//
// phase/130/check.go proves the tests were dead from the input -- every
...
func init() { check.Register("whim130", Check) }
...
func Check(w io.Writer, args []string) error {
	c, err := check.NewCore(w, args, "whim130", "sentinel")
	if err != nil {
		return err
	}
	r := c.R
	...
	got := check.CountLines(ob) - check.CountLines([]byte(c.New))
	if got != want {
	...
	sc, _, _ := check.Stream(nw, ctl, nil)
	sg, _, _ := check.Stream(nw, [][]byte{seed, []byte("g;g;"), []byte(":q!\r")}, nil)
	if sc == sg {
