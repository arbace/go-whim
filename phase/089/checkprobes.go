...
func w89Probes(r *check.Rep, old, bin string) error {
	...
	for _, name := range []string{"write_roundtrip", "w_named", "sav_named", "up_named", "wq_named", "x_named"} {
		...
		if len(o.oF) != 1 || o.oF["out.txt"] != want {
			fail = append(fail, fmt.Sprintf("%s: the input binary left %s, not a %d-byte out.txt, so this proves nothing about writing",
				name, check.PyDict(o.oF), want))
		}
		if len(o.nF) > 0 {
			fail = append(fail, fmt.Sprintf("%s: the new binary left %s on the disk", name, check.PyDict(o.nF)))
		}
	}
	...
	if o := by["sav_a"]; len(o.oF) != 1 || o.oF["a"] != 6 {
		fail = append(fail, fmt.Sprintf("sav_a: the input binary left %s, not a 6-byte `a`", check.PyDict(o.oF)))
	}
