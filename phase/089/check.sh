...
# See phase/089/edit.sh, and GOALS.md.
#
# Usage: phase/089/check.sh <work-dir> <state-dir>     (run from the repository root)
#
# Runs after phase/089/edit.sh and the sweep tools/phaserun.sh runs between them,
...
#    phase/085/check.sh drives a pty with `:wq` and reads the file back too, and
...
# THE BODY IS GO: internal/check/whim89.go and internal/check/whim89probes.go, run through tools/st.sh.
...
work=${1:?usage: phase/089/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/089/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim89 "$work" "$state"
