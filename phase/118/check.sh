...
# See phase/118/edit.sh, and GOALS.md II.4c.
#
# Usage: phase/118/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/118/edit.sh and the sweep tools/phaserun.sh runs between them, and
...
work=${1:?usage: phase/118/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/118/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim118 "$work" "$state"
