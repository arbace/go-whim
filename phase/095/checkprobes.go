...
func w95Probes(r *check.Rep, old, bin string) error {
	...
	for _, name := range []string{"readonly", "fsync", "prompt", "undoreload"} {
		if !strings.Contains(o.oS, name) {
			fail = append(fail, fmt.Sprintf("set_all: the input binary did not list %s, so this proves nothing", check.CutilRepr(name)))
		}
		if strings.Contains(o.nS, name) {
			fail = append(fail, fmt.Sprintf("set_all: %s is still listed", check.CutilRepr(name)))
		}
	}
	...
	for _, p := range []struct{ Name, want string }{{"shm_F", "shortmess=F"}, {"cpo_g", "cpoptions=g"}} {
		...
		for _, s := range []struct{ Tag, stream string }{{"the input binary", o.oS}, {"this one", o.nS}} {
			if !strings.Contains(s.stream, p.want) || strings.Contains(s.stream, "E539") {
				fail = append(fail, fmt.Sprintf("%s: %s did not accept the inert letter silently and answer %s", p.Name, s.Tag, check.CutilRepr(p.want)))
			}
		}
	}
