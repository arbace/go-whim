...
# See phase/092/edit.sh, and GOALS.md.
#
# Usage: phase/092/check.sh <work-dir> <state-dir>     (run from the repository root)
#
# Runs after phase/092/edit.sh and the sweep tools/phaserun.sh runs between them,
...
# THE BODY IS GO: internal/check/whim92.go and internal/check/whim92evidence.go, run through tools/st.sh.
...
work=${1:?usage: phase/092/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/092/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim92 "$work" "$state"
