...
# See phase/102/edit.sh, and GOALS.md.
#
# Usage: phase/102/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/102/edit.sh and the sweep tools/phaserun.sh runs between them,
...
# THE BODY IS GO: internal/check/whim102.go, run through tools/st.sh.
...
work=${1:?usage: phase/102/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/102/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim102 "$work" "$state"
