...
# See phase/091/edit.sh, and GOALS.md.
#
# Usage: phase/091/check.sh <work-dir> <state-dir>     (run from the repository root)
#
# Runs after phase/091/edit.sh and the sweep tools/phaserun.sh runs between them,
...
# THE BODY IS GO: internal/check/whim91.go and internal/check/whim91probes.go, run through tools/st.sh.
...
work=${1:?usage: phase/091/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/091/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim91 "$work" "$state"
