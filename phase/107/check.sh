...
# See phase/107/edit.sh, and GOALS.md II.4c.
#
# Usage: phase/107/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/107/edit.sh and the sweep tools/phaserun.sh runs between them, and
...
# THE BODY IS GO: internal/check/whim107.go, run through tools/st.sh.
...
state=${2:?usage: phase/107/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim107 "$work" "$state"
