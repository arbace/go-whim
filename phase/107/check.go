...
func Check(w io.Writer, args []string) error {
	...
	oldSize, newSize := check.SizeOf(oldBin), check.SizeOf(T("new"))
	...
	ob := []byte(check.ReadFile(oldBin))
	if !bytes.Equal(ob, []byte(check.ReadFile(T("new")))) {
	...
	r.Say("THE BINARY IS BYTE-IDENTICAL, %d bytes either side -- tier 1 of CLAUDE.md's verification table, and the whole of this phase's evidence for the 133 it removed and respelled.  A byte-identical binary subsumes every screen case, every Ex-command row, every command line and every pty scenario at once, because the program that would be run is the same program; the core delta at phase 107 runs next and corroborates rather than proves", newSize)
	if jC4.wait().Err != nil {
		r.Say("the control c4 did not build:")
		for _, l := range check.Head(strings.Split(strings.TrimRight(jC4.Out, "\n"), "\n"), 5) {
			fmt.Fprintln(w, "               "+l)
		}
		return harness.ErrReported
	}
	c4b := []byte(check.ReadFile(T("c4")))
...
func z24Params(r *check.Rep, oldC, pOld, pNew string) error {
	...
	if len(stray) > 0 {
		...
		for _, x := range head2(stray, 5) {
			s = append(s, fmt.Sprintf("(%d, %s)", x.line, check.PyRepr(x.Name)))
		}
...
func z24FallC(r *check.Rep, NL []string, ctl, syn string) error {
	...
	if len(quiet) != len(sites)-n {
		var s []string
		for _, x := range quiet {
			s = append(s, fmt.Sprintf("%d %s", x.s, check.PyRepr(x.l)))
		}
	...
	var qs []string
	for _, x := range quiet {
		qs = append(qs, fmt.Sprintf("line %d after %s", x.s, check.PyRepr(x.l)))
	}
...
func z24C23C(w io.Writer, r *check.Rep, newC, tmp string, j11o, j11n *z24Job) error {
	...
	indent4 := func(s string) {
		for _, l := range check.Head(strings.Split(strings.TrimRight(s, "\n"), "\n"), 4) {
			fmt.Fprintln(w, "               "+l)
		}
	}
	...
	j11o.wait()
	j11n.wait()
	eOld, eNew := check.CountLinesWith([]byte(j11o.Out), "error:"), check.CountLinesWith([]byte(j11n.Out), "error:")
...
func z24Buys(w io.Writer, r *check.Rep, NL []string, tmp string) error {
	...
	if !strings.Contains(fe, "expects argument of type") {
		...
		for _, l := range check.Head(strings.Split(strings.TrimRight(fe, "\n"), "\n"), 4) {
			fmt.Fprintln(w, "               "+l)
		}
		return harness.ErrReported
	}
