...
What the schedule must respect (phase/STAGES.md defines the two words).
...
by running phase/091/check.go on the tree phase 92 leaves, which gives five
...
phase takes every one of them to 0.  Measured by running phase/092/check.go on
...
the `:q` phase takes -- phase/089/check.go:158 and :422-424 are the first of them.
This phase takes it.  Measured by running phase/093/check.go on the tree phase 94
...
both rows.  Measured by running phase/094/check.go on the tree phase 95 leaves:
...
and frees all four symbols.  Measured by running phase/095/check.go on the tree
...
apart       108 109
```
MEASURED rather than predicted, by running phase/109/check.go on the tree phase 110
...
apart       109 110
```
MEASURED rather than predicted, by running phase/110/check.go on the tree phase 111
...
What phase 90's edit needs of the text it is handed (phase/STAGES.md defines the
...
```
  ...
  NO `need 111`, AND IT WAS MEASURED RATHER THAN ASSUMED.  Phase 110's edit output was
                 ...
                 functions, 0 variables, 0 types, 0 fields, 0 enumerators, canon
                 settled, the file not one line shorter than the edit left it.
phase/104/check.go pins `msg_puts_printf` at 3 mentions -- "a prototype, a
...
phase/116/check.go is ONE file and a stage of more than one phase is made of split
...
MEASURED, by putting this phase's own tar in .build and running phase/116/check.go
...
running phase/127/check.go on the q128 tree with phase 127's own state directory:
...
```
  NO `apart 122 123` AND NO `need 123`, AND NEITHER IS AN OMISSION.  Phase 123 is a
                 WHOLE-PHASE program, phase/123/check.go -- phase 86's shape and
...
RATHER THAN THINGS NOBODY LOOKED AT.
  ...
  NO `apart`.    Zero ran every phase as its own stage, so an `apart` line here would be a
                 ...
                 phase 117's reason rather than for any of its own: phase/123/check.go is
...
phase 107 having gone to `dialect` below.  phase/106/edit.go and
phase/106/check.go still say "phase 109" where they mean the move, and they are
...
spelling (phase/STAGES.md, and zero's own `apart 105 106`).  phase/125/check.go
...
`need` exists for (phase/STAGES.md, phase 54 after 53).
```
