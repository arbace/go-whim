...
# See phase/119/edit.sh, and GOALS.md II.4c.
#
# Usage: phase/119/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/119/edit.sh and the sweep tools/phaserun.sh runs between them, and
...
# THE BODY IS GO: internal/check/whim119.go, run through tools/st.sh.
...
work=${1:?usage: phase/119/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/119/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim119 "$work" "$state"
