...
# See phase/100/edit.sh, and GOALS.md.
#
# Usage: phase/100/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/100/edit.sh and the sweep tools/phaserun.sh runs between them,
...
# THE BODY IS GO: internal/check/whim100.go and internal/check/whim100evidence.go, run through tools/st.sh.
...
work=${1:?usage: phase/100/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/100/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim100 "$work" "$state"
