#!/bin/sh
# Whim phase 22 -- the working directory is where it started.  See GOAL.md.
#
# Usage: phase/022/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
work=${1:?usage: phase/022/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

# --- cut the entry points -------------------------------------------------
tools/st.sh nochdir "$f"

# tools/phaserun.sh sweeps next, then runs phase/022/check.sh.
