#!/bin/sh
# Whim phase 51 -- a byte that is not UTF-8 is kept as it is.  See GOAL.md.
#
# Usage: phase/051/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
work=${1:?usage: phase/051/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

tools/st.sh keepbytes "$f"

# tools/phaserun.sh sweeps next, then runs phase/051/check.sh.
