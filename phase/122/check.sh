...
# See phase/122/edit.sh, and GOALS.md.
#
# Usage: phase/122/check.sh <work-dir> <state-dir>   (run from the repository root)
#
# Runs after phase/122/edit.sh and the sweep tools/phaserun.sh runs between them,
...
# THE BODY IS GO: internal/check/whim122.go, run through tools/st.sh.
...
work=${1:?usage: phase/122/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/122/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim122 "$work" "$state"
