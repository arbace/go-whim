...
// See phase/122/edit.go, and GOALS.md.
//
// Runs after phase/122/edit.go and the sweep tools/phaserun.sh runs between them,
...
func Check(w io.Writer, args []string) error {
	...
	for _, x := range was {
		...
		if len(pat.FindAllStringIndex(Body, -1)) != 1 {
			return die(CT, "the input's switch does not hold one `case %s: ... break;` arm", check.PyRepr26(x))
		}
	...
	load := func(p string) ([]string, map[string]string) {
		var order []string
		m := map[string]string{}
		for _, l := range strings.SplitAfter(check.ReadFile(p), "\n") {
	...
	// this phase's key.  Do not delete it.
	rdec := func(p string) string { return harness.DecodeReplace([]byte(check.ReadFile(p))) }
	...
	argvErr := func() string {
		...
		for _, name := range wk {
			...
			if want && a == b {
				badA = append(badA, fmt.Sprintf("%s spells a removed option and did not move", check.PyRepr26(name)))
			}
			if !want && a != b {
				badA = append(badA, fmt.Sprintf("%s moved and spells no removed option", check.PyRepr26(name)))
			}
			if want {
	...
	if dl := check.DiffRQ(T("rec-c5"), T("rec-new")); len(dl) > 0 {
	...
	if len(check.DiffRQ(T("rec-c5"), T("rec-old"))) == 0 {
	...
	if dl := check.DiffRQ(T("rec-c1"), T("rec-new")); len(dl) > 0 {
	...
	filesWith := func(dir, tok string) int {
		n := 0
		for _, Rel := range check.WalkFiles(dir) {
			if strings.Contains(check.ReadFile(filepath.Join(dir, Rel)), tok) {
				n++
			}
		}
		return n
	...
	rowsOld := 0
	for _, l := range strings.Split(check.ReadFile(filepath.Join(T("rec-i-old"), "ref-argv.txt")), "\n") {
	...
	ask := func(binary, name string) string {
		home, _ := os.MkdirTemp("", "whim122-home-")
		defer os.RemoveAll(home)
		d, _ := os.MkdirTemp("", "whim122-")
	...
	colErr := func() string {
		...
		return fmt.Sprintf("OKthe %s test does NOT fold and this phase does not pretend it does: forcing it "+
			...
			"%s answer identically on both binaries -- %s", check.PyRepr26(needle), len(up), len(rows),
