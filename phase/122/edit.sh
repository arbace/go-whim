...
# See GOAL.md.
#
# Usage: phase/122/edit.sh <work-dir> <state-dir>     (run from the repository root)
#
...
# phase/122/check.sh measures all of that with the same marker in the same place
...
# phase/088/edit.sh takes them: the check needs the old binary to show what `-T`
...
work=${1:?usage: phase/122/edit.sh <work-dir> <state-dir>}
state=${2:?usage: phase/122/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
tools/st.sh edit whim122 "$f"

# NOT create_cmdidxs --check, for phase/085/edit.sh's reason: the derived
...
# tools/phaserun.sh sweeps next, then runs phase/122/check.sh.
