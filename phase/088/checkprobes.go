...
// w88Probes is section 6: 22 command lines recorded on BOTH binaries, six of
...
func w88Probes(r *check.Rep, old, bin string) error {
	quit := []byte("\x1b:q!\r")
	esc := []byte("\x1b")
	only := [][]byte{quit}
	...
	for i, p := range probes {
		...
		go func(i int, p check.W87Probe) {
			defer wg.Done()
			outs[i] = outcome{p.Name, check.CoreRecord(old, p.Args, p.Keys), check.CoreRecord(bin, p.Args, p.Keys), p.Differ}
		}(i, p)
	}
	wg.Wait()
	...
	if strings.Contains(o.n.Text, "Too many edit arguments") || !strings.Contains(o.n.Text, w88Unk) {
	...
	if strings.Contains(o.n.Text, "blocked") || !strings.Contains(o.n.Text, w88Unk) {
...
// w88Pty is section 7: a file on the command line on a real terminal, where the
// old binary edits it and the new one refuses before the screen exists.
func w88Pty(r *check.Rep, old, bin string) error {
