...
func Check(w io.Writer, args []string) error {
	...
	tmp, err := os.MkdirTemp("", "whim99-check-")
	if err != nil {
		return err
	}
	...
	// `whim-vim.c` in work give identical bytes, because whim-vim.c names no
	...
	for _, s := range z16Supplied {
		...
		if len(live) > 0 {
			r.Bad("<%s> was removed and whim-vim.c still names %s",
	...
	if _, err := os.Stat(bin); err == nil {
		return fmt.Errorf("  build        the clean did not remove whim-vim, so a 'rebuild' " +
