...
# See phase/099/edit.sh, and GOALS.md.
#
# Usage: phase/099/check.sh <work-dir> <state-dir>    (run from the repository root)
#
# Runs after phase/099/edit.sh and the sweep tools/phaserun.sh runs between them,
...
#     GOALS.md II.4b's invariant, which <fcntl.h> and <sys/stat.h> leaving makes
...
# THE BODY IS GO: internal/check/whim99.go, run through tools/st.sh.
...
work=${1:?usage: phase/099/check.sh <work-dir> <state-dir>}
state=${2:?usage: phase/099/check.sh <work-dir> <state-dir>}
exec tools/st.sh check whim99 "$work" "$state"
