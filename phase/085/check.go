...
func Check(w io.Writer, args []string) error {
	...
	if n := check.Occurrences(`\bui_delay\(`, src); n != 10 {
	...
	if n := check.Occurrences(`\bisatty\(`, src); n != 5 {
	...
	scan := check.AwkRange(src, `^command_line_scan\(mparm_T`, `^\}$`)
	...
	// editor.
	check.CopyExec(filepath.Join(state, "old"), filepath.Join(d, "old", "vim"))
	check.CopyExec(bin, filepath.Join(d, "new", "vim"))
	...
	if check.PipeJoin(check.ReadFile(filepath.Join(d, "new", "f.txt"))) != "one|two|three|" {
	...
	for _, side := range []string{"old", "new"} {
		...
		if check.PipeJoin(check.ReadFile(filepath.Join(d, side, "-x.txt"))) != "z-dd|" {
		...
		if g := check.PipeJoin(check.ReadFile(filepath.Join(d, side, "c.txt"))); g != "one-PLUS|two|" {
		...
		if g := check.PipeJoin(check.ReadFile(filepath.Join(d, side, "T.txt"))); g != "one-T|two|" {
