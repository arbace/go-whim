...
#
# Usage: phase/121/edit.sh <work-dir> <state-dir>     (run from the repository root)
#
...
# -- no surviving row carries that name -- and phase/121/check.sh proves it again
...
# format string and the name move together, as phase/095/edit.sh's `[RO]` did,
...
work=${1:?usage: phase/121/edit.sh <work-dir> <state-dir>}
state=${2:?usage: phase/121/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"
...
# tools/phaserun.sh sweeps next, then runs phase/121/check.sh.
