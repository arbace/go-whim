...
func z38Blocks(p string) ([]string, map[string]string) {
	...
	cur, have := "", false
	for _, line := range strings.SplitAfter(check.ReadFile(p), "\n") {
		if line == "" {
			continue
		}
...
func Check(w io.Writer, args []string) error {
	...
	if _, e := os.Stat(newBin); e == nil {
		(&check.Rep{Tag: "build", W: w}).Say("the clean did not remove whim-vim, so nothing below would be a recording of this phase")
		return harness.ErrReported
	}
	...
	for _, x := range nrows {
		...
		if x.Text != was.Text {
			return stop("the row for %s is not the row it was: %s became %s", check.PyRepr26(x.Name),
				check.PyRepr26(strings.TrimSpace(was.Text)), check.PyRepr26(strings.TrimSpace(x.Text)))
		}
	}
	...
	for _, x := range tabs {
		kept := false
		for _, o := range orows {
			if o.tab == x && check.Contains(nnames, o.Name) {
				kept = true
			}
		}
		if kept {
	...
	for _, fn := range []string{"set_termname", "termcapinit"} {
		...
		for _, l := range nlits {
			s := newT[l[0]+1 : l[1]]
			if lo <= l[0] && l[0] < hi && check.Contains(onames, s) {
				got[s] = true
			}
		}
		...
		if !check.Contains(nnames, gs[0]) {
			return stop("%s() names %s, which this phase removed: a name written outside the "+
				"table must be a row the table still has", fn, check.PyRepr26(gs[0]))
		}
		named[fn] = gs[0]
	}
	...
	r.Say("set_termname()'s fallback, termcapinit()'s compiled default and "+
		...
		"after this phase there is ONE name the editor falls back to", check.PyRepr26(named["set_termname"]))
	...
	fbFail := func() error {
		for _, what := range []string{"ref-excmds.txt", "ref-argv.txt", "ref-pty.txt", "ref-term.txt"} {
			...
			if what == "ref-term.txt" {
				if check.ReadFile(a) != check.ReadFile(b) {
		...
		sort.Strings(sa)
		for _, c := range sa {
			if check.ReadFile(filepath.Join(T("rec-new"), "screen", c)) != check.ReadFile(filepath.Join(T("rec-c1"), "screen", c)) {
		...
		for _, m := range moved {
			...
			if fa == nil || fbm == nil || nb >= na {
				return fmt.Errorf("%s draws no less with the repair left out", check.PyRepr26(m.k))
			}
		}
		return nil
	}
	...
	var kr []string
	for _, m := range moved {
		kr = append(kr, check.PyRepr26(m.k))
	}
	...
	if dl := check.DiffRQ(T("rec-c2"), T("rec-new")); len(dl) > 0 {
	...
	countMarked := func(dir string) int {
		...
		n := 0
		for _, e := range ents {
			if strings.Contains(check.ReadFile(filepath.Join(dir, e.Name())), "XTERMFAMILY") {
				n++
			}
		}
		return n
	...
	wgT.Wait()
	os.WriteFile(T("term-new"), []byte(check.ReadFile(filepath.Join(T("rec-new"), "ref-term.txt"))), 0o644)
	rt := &check.Rep{Tag: "terminals", W: w}
	rt.Say("the nineteen rows, before and after -- ztermcheck, `+set term={name}` on a real pty (phase 116):")
	ta := strings.Split(check.ReadFile(T("term-old")), "\n")
	tb := strings.Split(check.ReadFile(T("term-new")), "\n")
	...
	ruleErr := func() (string, error) {
		...
		for k := range was {
			...
			if check.Contains(GONE, name) {
				mvd = append(mvd, name)
				if !check.Contains(strings.Fields(a), "term="+name) {
					bad = append(bad, fmt.Sprintf("%s lost its row but did not resolve BEFORE: %s", check.PyRepr26(name), a))
				}
				fs := strings.Fields(b)
				hasE, termTok := false, ""
				for _, t := range fs {
				...
				if !hasE || termTok != def {
					bad = append(bad, fmt.Sprintf("%s lost its row and answered %s, where a refusal and %s "+
						"were due", check.PyRepr26(name), b, def))
				}
			} else if a != b {
				bad = append(bad, fmt.Sprintf("%s kept its row and moved: %s -> %s", check.PyRepr26(name), a, b))
			}
		}
	...
	one := func(path, name, want string) (string, string, error) {
		...
		if len(mvd) != 1 {
			return "", "", fmt.Errorf("%s the %s row moved %d rows, and exactly 1 was due", want, check.PyRepr26(name), len(mvd))
		}
		a, b := mvd[0][0], mvd[0][1]
		got := a[strings.Index(a, "'")+1 : strings.LastIndex(a, "'")]
		if got != name {
			return "", "", fmt.Errorf("%s the %s row moved the %s row instead", want, check.PyRepr26(name), check.PyRepr26(got))
		}
	...
	ra.Say("deleting the %s row from THIS PHASE'S OWN table moves exactly 1 of %d rows, "+
		...
		"or nine would have been seen", check.PyRepr26(c3Name), len(base), a3, b3, check.PyRepr26(c4Name), a4, b4)
	...
	for _, tt := range tests {
		...
		if tt[3] == "==" && len(tt[1]) != n {
			badT = append(badT, fmt.Sprintf("(%s, %s)", check.PyRepr26(tt[1]), check.PyRepr26(tt[2])))
		}
	}
	if len(badT) > 0 {
