...
#
# Usage: phase/044/edit.sh <work-dir> <state-dir>      (run from the repository root)
#
...
work=${1:?usage: phase/044/edit.sh <work-dir> <state-dir>}
f="$work/whim-vim.c"

tools/st.sh retire "$f" '!' sort uniq retab left center right
tools/st.sh edit whim44 "$f"

# tools/phaserun.sh sweeps next, then runs phase/044/check.sh.
