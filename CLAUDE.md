...
- **The input is one file**, `slim-vim.c`: vim 9.2 as a single C23 translation
  ...
  arbace/slim-vim.
- **whim** (`whim.mk`) removes capability on purpose, 153 phases from 180,870
  ...
  - **phases 129-152** (the last sections of Part II) remove from the core what
...
- **Stages** (`pipes/whim.stages`): a run of phases whose end is the only
  ...
  stage (`stage A-B each`, phases 87-152) sweeps after every edit, then runs every
...
`WHIM-GOAL.md` Part II, *Adding a phase*, has the process; the next phase is 153. In
