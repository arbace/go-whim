...
func (g *gen) write(dir string) error {
	...
	if err := os.WriteFile(filepath.Join(dir, "types.go"), []byte("// Code generated by `go tool whim gen` from editor.c: the types of the transpilation.\n\n"+g.typesText()), 0o644); err != nil {
		return err
	}
	if err := os.WriteFile(filepath.Join(dir, "globals.go"), []byte("// Code generated by `go tool whim gen` from editor.c: every file-scope object and every\n// block-scope static, hoisted as <function>_<name>, with its type and no initializer.\n\n"+g.globalsText()), 0o644); err != nil {
		return err
	}
	// sigs.md is DATA, so it is lowercase and its data is in a fenced block:
	// the signature of every function the transpilation writes, for whoever
	// writes a body against it (internal/gen/CONVENTIONS.md).
	sigs := "Generated by `go tool whim gen` from editor.c: the Go signature of every function\n" +
		"the transpilation writes.  Copy the one you need (internal/gen/CONVENTIONS.md).\n\n" +
		"```go\n" + strings.Join(dedupe(g.sigs), "\n") + "\n```\n"
	return os.WriteFile(filepath.Join(dir, "sigs.md"), []byte(sigs), 0o644)
}
