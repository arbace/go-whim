...
4. **Kept current.** `editor.clj` generated by `whim gen` beside `editor.go`
   ...
   **Done**: `whim gen` writes `cljeditor/src/whim/editor.clj` (76,975 lines)
...
| `cljeditor.go` | `braaam.go` | `Generate` (the cut, the backend), `Compile`, `AOT`, `Jar`, `Train`, `WriteLauncher` |
...
Java. `bin/whim-clj` is `exec java <braaam's flags> -XX:AOTCache=...
