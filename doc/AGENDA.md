...
1. **braaam: parentheses by precedence, masks where needed.** 11,127
   redundant parentheses; 1,200 `& 0xff` where the byte is only compared.
   `whim java --same-classes` is the proof for the parentheses.
2. **braaam: locals where the C declares them.** 3,449 are hoisted to the
   top of the method with a throwaway zero.
3. **A C phase for `pos_T`'s address-taken members** (both surveys). A
...
4. **vijure: the printer's noise** (`CLOJURE-IDIOMS.md`). clj-kondo: 2,993
...
5. **vijure: the four nesting rules.** Better lowering makes 154 of 334
...
6. **vijure: kebab-case names and `?` predicates**, then address-taken
