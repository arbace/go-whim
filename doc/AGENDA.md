...
1. **braaam: locals where the C declares them.** 3,449 are hoisted to the
   top of the method with a throwaway zero.
2. **A C phase for `pos_T`'s address-taken members** (both surveys). A
...
3. **vijure: the printer's noise** (`CLOJURE-IDIOMS.md`). clj-kondo: 2,993
...
4. **vijure: the four nesting rules.** Better lowering makes 154 of 334
...
5. **vijure: kebab-case names and `?` predicates**, then address-taken
