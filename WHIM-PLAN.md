# WHIM-PLAN.md — how the whim pipeline could be better formed, and how phases 83 onwards were planned

Part I is the plan that reorganised phases 0 to 82 into stages and packages. Part II is
the plan phases 83 onwards were built from.

# Part I — the pipeline, better formed
...
# Part II — how phases 83 onwards lose the filesystem, the streams and their own `main()`

A plan, kept as written: it was `ZERO-PLAN.md` when phases 83 onwards were a pipeline of
their own, zero, numbered from 0, and `zero-vim.c` was its product — the `whim-vim.c`
of today. The phase numbers below are the one pipeline's; the `P2`…`P12` labels are the
plan's own names for phases it proposed, which the section *II.3b* maps to what was
built. Its sections are numbered II.1 to II.6 so that a citation says which part it
means.

`WHIM-GOAL.md` Part II was the charter, and it had two phases in it, 83 and 84, when
...
past it.** What actually happened is in `WHIM-GOAL.md`, one *What zero-vim is after
phase N* block per phase; the current one reads 78,666 lines, 1,734 functions, 14
undefined symbols and a 760,424-byte binary, and argv is `+{command}` alone. §II.4 below is
...
compile line, three times, requiring the three runs to be identical — `WHIM-GOAL.md`
...
The programs are zero-only files, named by `pipes/whim83.sh` and
...
which is `WHIM-GOAL.md` core rule 9's requirement and the precedent `zerodelta.sh` set.
...
**This is a change to phase 83's program**, not a new phase: `pipes/whim83.sh` is
...
for slim and whim (`WHIM-GOAL.md` core rule 9: a zero tool is a new file, never an edit
to a hashed one).
...
`zcompare.py`, 1,144 lines, named by nothing whim or slim runs. `WHIM-GOAL.md`'s
...
between rows 2 and 3 as phase 86. This plan is done; what is left of it is §II.4c. `WHIM-GOAL.md` is what each one did; where this
...
`ex_ni`, so a zero phase deletes the row and the enumerator (§II.3a) and
...
* ~~**the includes.**~~ **BUILT, AS ZERO PHASE 16, AND THIS BULLET IS WRONG TWICE.**
  ...
  **79,598 lines and 15**: the typedef sits between two blank lines and one of them
  ...
  `SOURCE_DATE_EPOCH=0`. `WHIM-GOAL.md`'s charter now says a phase may remove a
...
undefined symbols, which is what `WHIM-GOAL.md`'s Phase 84 records.
