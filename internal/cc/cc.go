...
#define __builtin_bit_cast(type, expr) (((union { __typeof__(expr) __ccgo_from; type __ccgo_to; }){ .__ccgo_from = (expr) }).__ccgo_to)
...
#define __has_include_next(xxx) __has_include_next(xxx)
...
func fixArgs(args *[]string, msg string) (r bool) {
	...
	// Catch-all for wordings the tags above miss. Compilers quote the offending
	// flag no matter how they phrase the rejection, e.g. clang on arm64 hosts
	// (darwin/arm64, windows/arm64, openbsd/arm64) says "unsupported option
	// '-mlong-double-64' for target '...'". If the failing diagnostic quotes a
	// flag we actually passed, drop it. Compilers delimit it with straight
	// ('...') or, for GCC, curly (‘...’) quotes.
	for _, v := range *args {
...
// lookPath wraps exec.LookPath and additionally handles MSYS / git-bash style
// /<drive>/... paths on Windows by translating them to <drive>:\... before
// retrying. On failure it returns the original exec.LookPath error so callers
// can decide what to do next.
func lookPath(s string) (path string, err error) {
	if path, err = exec.LookPath(s); err == nil {
		return path, nil
	}

	if runtime.GOOS == "windows" && len(s) >= 4 && s[0] == '/' && s[2] == '/' {
		if c := s[1]; c >= 'a' && c <= 'z' || c >= 'A' && c <= 'Z' {
			win := string(c) + `:\` + filepath.FromSlash(s[3:])
			if p, e := exec.LookPath(win); e == nil {
				return p, nil
			}
		}
	}
...
// c23Keywords are the C23 spellings of keywords the parser knows under their
// C11 spellings. They are added to the keyword set when the compiler reports
// C23 - __STDC_VERSION__ >= 202000L, gcc's C2x value included - because from
// then on <assert.h>, <stdalign.h>, <stdbool.h> and <threads.h> no longer
// define them as macros. gcc 15 defaults to C23.
var c23Keywords = map[string]rune{
	"alignas":       rune(ALIGNAS),
	"alignof":       rune(ALIGNOF),
	"bool":          rune(BOOL),
...
// if it is not defined there.
func stdcVersion(predefined string) int64 {
	const prefix = "#define __STDC_VERSION__ "
	for _, l := range strings.Split(predefined, "\n") {
		if strings.HasPrefix(l, prefix) {
			n, _ := strconv.ParseInt(strings.TrimSuffix(strings.TrimSpace(l[len(prefix):]), "L"), 10, 64)
			return n
		}
	}
	return 0
...
func newConfig(opts []string) (cc, predefined string, includePaths, sysIncludePaths []string, keywords map[string]rune, err error) {
	...
	for _, cc = range []string{os.Getenv("CC"), "cc", "gcc"} {
		if Dmesgs {
			Dmesg("trying cc candidate: %q", cc)
		}
		if cc == "" {
			continue
		}
		...
		orig := cc
		cc, err = lookPath(orig)
		if err != nil {
			if Dmesgs {
				Dmesg("lookPath(%q) FAIL: %v", orig, err)
			}
			continue
		}

		if Dmesgs {
			Dmesg("lookPath: %q -> %q", orig, cc)
		}
	again:
		...
		if err != nil {
			if Dmesgs {
				Dmesg("cpp probe %q FAIL: %v", cc, err)
			}
		...
		if stdcVersion(predefined) >= 202000 {
			clone()
