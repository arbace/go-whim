...
`internal/phase/116/check.go`, one whole program, `stage 116`, `package harness`. The second phase
...
its **own output**. `internal/phase/083/check.go` does the opposite and enforces it: the baselines
...
and **both paths are needed**: measured, with only `.cache/r0` removed `internal/phase/083/check.go`
...
The baseline and every phase's recording move **together**, and `internal/phase/116/check.go`
...
`internal/phase/116/check.go` is one file, so the schedule is refused before any check runs:
...
is what the phase list living in `internal/phase/STAGES.md` rather than in
