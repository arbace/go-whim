...
func init() { edit.Register("whim168", Edit) }

// Edit writes `return x;` for every `goto L;` whose label L marks `return x;`:
// at the goto, the return does what the jump would lead to, in the same state.
// The tree LOCATES (cc.Parse); the text is what is rewritten, the return's
// own text copied to the goto.
//
...
// statement before it always jumps, since nothing reaches it then (phase
// 164's rule).
...
// terminates: control never runs off the end of the item (phase 164's rule).
