...
`internal/phase/103/edit.go` and `internal/phase/103/check.go`, `stage 103`, `package host`.
...
`tools/zhostonly.py` is new, Part II-only, and named by `internal/phase/103/check.go` alone: 43
...
`internal/phase/103/delta.md` gets a comment block and no line. Two full recordings either side
