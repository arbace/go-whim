...
`internal/phase/113/edit.go` and `internal/phase/113/check.go`, `stage 113`, `package host`.
...
* **`exit_scroll()`'s printf arm is ALIVE, and phase 104 was wrong to name it a follow-up
  beside `msg_puts_printf()`.** `internal/phase/104/check.go` says the two "fire in ZERO of
