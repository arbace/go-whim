...
`internal/phase/102/edit.go` and `internal/phase/102/check.go`, `stage 102`, `package host`.
...
`internal/phase/102/check.go` **builds the `sigsetjmp` variant on every run** and requires
...
`internal/phase/102/delta.md` gets a comment and no line, and it is phase 101's kind. Everything
