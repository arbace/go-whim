...
cut as "79,599 lines and **16 directives**", and `internal/phase/096/edit.go` names two
...
rationale`, because `internal/phase/096/edit.go` names `<sys/stat.h>` and `<fcntl.h>`,
...
cuts nothing. The second class exists for the closure swap (`WHIM_CLOSURE=1`,
`internal/sweep/closure.go`): the closure reads the typedef as a definition
