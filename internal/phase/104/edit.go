...
// not removed here -- phase 169 drops every header the file does not need, together
...
// in this editor is built in -- and internal/phase/104/check.go pins both halves of it, the
...
// remaining step and not this one.  What IS assertable, and what internal/phase/104/check.go
...
`
	...
	// <stdio.h> is unused from here, and stays for phase 169, which drops every
	...
	p.Sayf("%d -> %d lines.  Every byte that leaves this editor other than the screen "+
		...
		"<stdio.h> is unused, for phase 169 to drop", linesBefore, p.Lines(t))
	return t, nil
}
