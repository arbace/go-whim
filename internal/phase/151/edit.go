...
func Edit(text []byte, w io.Writer) ([]byte, error) {
	...
	var o []byte
	if o, err = p.Literal([]byte(s), "    char_u *def_val[2];\n", "    char_u      *def_str[2];\n    long        def_num[2];\n", 1, "a row holds its string defaults and its number defaults apart"); err != nil {
		return nil, err
	}
	s = string(o)
	if o, err = p.Literal([]byte(s), "options[opt_idx].def_val[VI_DEFAULT] = (char_u *)(long_i)val;", "options[opt_idx].def_num[VI_DEFAULT] = val;", 1, "a number default is stored as a number"); err != nil {
		return nil, err
	}
	s = string(o)
