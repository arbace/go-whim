...
// and `phasecheck` in internal/phase/089/check.go is where "it compiles" is
...
func init() { phase.Register("whim89", Edit) }
...
func Edit(text []byte, w io.Writer) ([]byte, error) {
	...
	left, holders, _, err := vimtext.CoreResidue(p, text, w89Six)
	if err != nil {
		return nil, err
	}
