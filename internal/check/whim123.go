...
func Whim123(w io.Writer, args []string) error {
	...
	reprobed, _, _ := z40Probe(readFile(T("reshape", "whim-vim.c")))
	...
	if sha256File(f) != before {
		(&rep{tag: "source", w: w}).say("whim-vim.c was modified by a phase that must not modify it")
		return harness.ErrReported
	}
	(&rep{tag: "source", w: w}).say("whim-vim.c unchanged, %d lines: r40 is its input's tree, and only the instrument moved", countLines([]byte(readFile(f))))
	return nil
}
