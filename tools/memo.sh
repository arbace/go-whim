#!/bin/sh
# The memoize, for one phase: a result, else the program.
#
# Usage: tools/memo.sh <unit> <work-dir> <build-dir>
#
...
# This repository has none.  Every phase here is a program, a program that
# fails is a failure to read and not a question to hand to an agent, and a
# phase that REFUSES is the check doing its job -- so a failure stops the pass,
# with the program's own report above it.
set -eu
set -o pipefail          # the tier-2 run is piped through an indenter

unit=${1:?usage: memo.sh <unit> <work-dir> <build-dir>}
work=${2:?}
build=${3:?}
. tools/pipeline.sh
first=${unit%-*}
...
if [ "$first" = 0 ]; then
    ...
    [ -f "$in_prev" ] || {
        echo "memo: unit $unit wants $TAG$(($first - 1)), which does not exist." >&2
        echo "      Phases must be contiguous; the phase list has a gap before $first." >&2
        exit 1
    }
    in_digest=$(cat "$in_prev")
fi
impl=$(tools/implhash.sh "$unit")
...
if [ "$first" = "$phase" ]; then
    ...
    name="$((phase - first + 1)) phases, one sweep: $(tools/phasename.sh "$first" 2>/dev/null || true) ... $(tools/phasename.sh "$phase" 2>/dev/null || true)"
fi
...
units=$(tools/stages.sh 2>/dev/null || true)
n=0; i=0
for _u in $units; do n=$((n + 1)); [ "$_u" = "$unit" ] && i=$n; done
[ "$first" = "$phase" ] && what=phase || what=stage
if [ "$i" = 0 ]; then of="[of $(tools/stages.sh --of "$phase")]"; else of="[$i/$n]"; fi
...
# --- tier 2: the code -----------------------------------------------------
tier=
if [ "$(tools/phaserun.sh --parts "$unit" | grep -c '')" -ge "$((phase - first + 1))" ]; then
    ...
    if tools/phaserun.sh "$unit" "$work" 2>&1 | sed 's/^/      /'; then
        tier=program
    else
        if [ "$first" = "$phase" ]; then
            echo "  tier 2       $TAG$phase FAILED"
        else
...
if [ -z "$tier" ] && [ "$first" != "$phase" ]; then
    for p in $(seq "$first" "$phase"); do
        ...
        tools/memo.sh "$p" "$work" "$build"
    done
    tier=phases
fi

# --- no tier below the program ---------------------------------------------
if [ -z "$tier" ]; then
    echo "  memo         $TAG$phase FAILED, and there is no agent tier here to fall" >&2
    echo "               through to: the phase program's report above is the reason." >&2
    exit 1
fi
