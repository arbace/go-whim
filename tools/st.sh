...
# NOT the implementation, because the builder says `find cmd internal` and
# `go build ./cmd/whimtools`, neither of which carries the trailing slash implhash's directory
...
# every phase's key:
#
#   cmd/                            the whole toolset: directories and not a
#   internal/                       list of files, because the build keys the
#   go.mod go.sum                   binary on every file under them
