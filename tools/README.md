...
| `stages.sh`, `packages.sh` | read and check `pipes/whim.stages`: the stages and their mode, `need` and `apart`; the packages and `uses` |
...
| `pipeline.sh` | the pipeline's parameters: tag, work directory, source, delta checker, `ZERO_FROM` (the phase where the second baselines begin), and the phase list, read from `pipes/whim.stages` |
...
| `whimdelta.sh`, `zerodelta.sh` | a stage's declared delta: `pipes/whim.delta` against `.reference/baselines` for phases 0-82, and from `ZERO_FROM` on `whimdelta.sh` hands the phase to `zerodelta.sh`, `pipes/zero.delta` against `.reference/zero-baselines` |
...
| `score.sh`, `residue.sh`, `phasename.sh` | reporting; `phasename.sh` reads a phase's name from its `WHIM-GOAL.md` heading, and `residue.sh` reports phase patches, of which there are none here |
