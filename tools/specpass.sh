...
if [ "${1:-}" = "--one" ]; then
    ...
    (
        ...
        # A PHASE 0 RECORDS ITS PIPELINE'S BASELINES, and on a fresh .reference/
        # it records them HERE, in the scratch root.  The sequential pass then
        ...
        for b in baselines core-baselines; do
            if [ -d ".reference/$b" ] && [ ! -L ".reference/$b" ] && [ ! -e "$root/.reference/$b" ]; then
                ...
                cp -r ".reference/$b" "$root/.reference/$b.part.$$"
                mv "$root/.reference/$b.part.$$" "$root/.reference/$b" 2>/dev/null || rm -rf "$root/.reference/$b.part.$$"
            fi
        done
    )
