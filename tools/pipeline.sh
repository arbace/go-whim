...
# subtly wrong.
#
# The boundary tag differs (q0.., r0..) so that one .cache/ can hold both
# without a key collision, and so that a stray q3 in a zero build is obviously
# wrong rather than plausibly right.  (arbace/slim-vim's own pipeline, which
# produces this one's input, tags its boundaries p.)
#
